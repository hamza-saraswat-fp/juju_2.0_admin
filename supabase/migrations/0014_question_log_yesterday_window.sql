-- 0014_question_log_yesterday_window.sql
--
-- Adds a 'yesterday' branch to get_question_log_phase2 so the daily Slack
-- digest can fetch yesterday's top category to match its "Juju yesterday"
-- copy. Without this, the digest fires at 9 AM and asks for '24h' (today
-- Chicago, midnight–9 AM only), which is sparse overnight traffic — so
-- the message advertises ~1 question instead of the real ~20.
--
-- Implementation: just shift today_chi_utc back by one day when
-- p_time_range = 'yesterday'. The existing window math below
-- ([today_chi_utc, today_chi_utc + 1 day)) then yields the right window
-- with no other code changes. Same outer/inner structure, same return
-- shape — purely additive for the dashboard, which never passes
-- 'yesterday'.
--
-- Hero RPC (get_question_log_metrics) doesn't need a sibling change: it
-- already exposes avg_messages_prior / auto_resolve_prior /
-- unique_users_prior, which for '24h' equal yesterday's full Chicago
-- calendar day. The edge function reads those instead.
--
-- Phase 3 RPC is intentionally untouched here. Weekly digest has the
-- same conceptual mismatch ('7d' = last 7 days including today) but is
-- a separate user complaint and gets its own follow-up migration.

DROP FUNCTION IF EXISTS public.get_question_log_phase2(text, text, text, text);
...
BEGIN
  ...
  -- 'yesterday' is just today shifted back by one day. The window math
  -- below (window_days = 1 → [today_chi_utc, today_chi_utc + 1 day))
  -- then resolves to yesterday's full Chicago calendar day with no
  -- other changes.
  IF p_time_range = 'yesterday' THEN
    today_chi_utc := today_chi_utc - interval '1 day';
  END IF;
